...
class Shop
{
    ...
    public function getAllbyID($id)
    {
        ...
        $temp = $GLOBALS["DB"]->dbquery("SELECT * FROM shop WHERE id ='" . $id . "'");
        if ($temp->num_rows == 0) {
            return false;
        }
    ...
    public function getUpdateByID($id)
    {
        ...
        $name = $_POST['name'];
        $update = $GLOBALS["DB"]->dbquery("UPDATE shop SET name='" . $name . "'
                       WHERE id='" . $id . "'");
        if ($update == true) {
            return true;
        } else {
    ...
    public function deleteByID($id)
    {
        ...
        $update = $GLOBALS["DB"]->dbquery("DELETE FROM shop  WHERE id='" . $id . "'");
        if ($update == true) {
            return true;
        } else {
            return false;
    ...
    {
        $temp = $GLOBALS["DB"]->dbquery("SELECT id FROM einkauf WHERE shop_id ='" . $id . "'");
        $result = $GLOBALS["DB"]->dbmakearry($temp);
        if ($temp->num_rows == 0) {
            return true;
        }
        if (count($result) > 0) {
            for ($i = 0; $i < count($result); $i++) {
                $update = $GLOBALS["DB"]->dbquery("UPDATE einkauf SET shop_id=1
    ...
    public function getAllbyShopID($id)
    {


        $result = $GLOBALS["DB"]->dbquery("SELECT * FROM einkauf WHERE archiv IS NULL AND shop_id ='" . $id . "'");


        //$result = $GLOBALS["DB"]->dbquery("SELECT * FROM einkauf WHERE shop_id ='".$id."'");
